...
$query = "SELECT id, given_name, sex, email, status_inquiry
          FROM applicant 
          WHERE service_selected = 'Registration' AND status_inquiry <> 'rejected' AND status_inquiry <> 'accepted'
...
<body>
  <div class="container">
    ...
    <h2 class="text-center mt-4">New Registration Applications</h2>
    ...
    <?php } else { ?>
        <div class="btn-group">
            <a href="view_details.php?id=<?php echo $row['id']; ?>" class="btn btn-info btn-action">View Details</a>
            <a href="request_missing_documents.php?id=<?php echo $row['id']; ?>" class="btn btn-warning btn-action">Ask for Missing Documents</a>
      ...
      <table class="table table-hover">
        ...
        <tbody>
          ...
          $query = "SELECT id, given_name, sex, email
                    FROM applicant 
                    WHERE service_selected = 'Registration' AND status_inquiry = 'accepted'
    ...
    <div class="rejected">
      ...
      <table class="table table-hover">
        ...
        <tbody>
          ...
          $query = "SELECT id, given_name, sex, email
                    FROM applicant 
                    WHERE service_selected = 'Registration' AND status_inquiry = 'rejected'
...
function confirmAccept(event, applicantId) {
    ...
    if (confirm("Do you want to generate the PDF file and send an email to the user?")) {
      window.location.href = "accept_application.php?id=" + applicantId;
    } else {
